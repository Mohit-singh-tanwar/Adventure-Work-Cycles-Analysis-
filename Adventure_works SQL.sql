...
SELECT COUNT(fullname) AS Total_Number_of_Customers FROM dimcustomer;
...
# 7.YEAR WISE SALES
SELECT YEAR(ArrangedOrderdateKey) AS year,
CONCAT(ROUND(SUM(SalesAmount)/1000000,2),"M") AS Sales FROM Total_sales
GROUP BY year
ORDER BY year;

# 8.MONTH WISE SALES
SELECT MONTHNAME(ArrangedOrderdateKey) AS Month,
CONCAT(ROUND(SUM(SalesAmount)/1000000,2),"M") AS Sales FROM Total_sales
GROUP BY Month
ORDER BY Sales;
...
SELECT SalesTerritoryGroup AS SalesGroup,
CONCAT(ROUND(SUM(SalesAmount)/1000000,2),"M") AS Sales ,
...
GROUP BY fullName ORDER BY Sales DESC LIMIT 10;
